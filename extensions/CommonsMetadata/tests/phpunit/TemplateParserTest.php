...
class TemplateParserTest extends \MediaWikiTestCase {
	...
	/**
	 * When the description is in language A, and we request language B, for which A is not a
	 * fallback, B should be returned.
	 * fixme this is a mixing of concerns - language resolution should be refactored into a
	 * separate class
	 */
	...
	/**
	 * When there is a single language template in the description,
	 * anything outside it should be ignored.
	 */
	public function testDescriptionOutsideLanguageTemplate() {
		$data = $this->parseTestHTML( 'outside_lang' );
		$this->assertFieldContainsString( 'ImageDescription', 'Williamsburg Historic District',
	...
	/**
	 * When there is only a single language in multilang mode,
	 * it should still be returned in an array.
	 */
	...
	/**
	 * In multilang mode, even when there is no language template used, we should still get the
	 * text, wrapped in an array.
	...
	/**
	 * There is no guarantee to return HTML, but surrounding whitespace and simple markup like
	 * a wrapping <p> should be removed.
	 */
	...
	public function testClassBasedInfotemplate() {
		...
		$this->assertFieldEquals( 'ImageDescription',
			'Tom Baker as the <a href="//en.wikipedia.org/wiki/Fourth_Doctor" ' .
				'title="Fourth Doctor">Fourth Doctor</a> in Doctor Who.',
			$data, TemplateParser::INFORMATION_FIELDS_KEY );
	}
	...
	public function testMultipleInfoTemplates() {
		...
		$this->assertFieldEquals( 'Artist',
			'<span class="fn value"><a href="//commons.wikimedia.org/wiki/User:El_Grafo" ' .
				'title="User:El Grafo">El Grafo</a></span>',
			$data, TemplateParser::INFORMATION_FIELDS_KEY );
	}
	...
	public function testSingleLicense() {
		...
		$this->assertFieldEquals( 'LicenseShortName', 'CC-BY-SA-3.0', $data,
			TemplateParser::LICENSES_KEY );
		// long name is called UsageTerms - bug 57847
		$this->assertFieldEquals( 'UsageTerms', 'Creative Commons Attribution-Share Alike 3.0',
			$data, TemplateParser::LICENSES_KEY );
		$this->assertFieldEquals( 'LicenseUrl', 'http://creativecommons.org/licenses/by-sa/3.0',
			$data, TemplateParser::LICENSES_KEY );
	...
	public function testMultiLicense() {
		...
		$this->assertFieldEquals( 'LicenseShortName', 'GFDL', $data,
			TemplateParser::LICENSES_KEY, 0 );
		$this->assertFieldEquals( 'LicenseShortName', 'CC-BY-SA-3.0', $data,
			TemplateParser::LICENSES_KEY, 1 );
		$this->assertFieldEquals( 'LicenseShortName', 'CC-BY-SA-2.5', $data,
			TemplateParser::LICENSES_KEY, 2 );

		$this->assertFieldEquals( 'UsageTerms', 'GNU Free Documentation License', $data,
			TemplateParser::LICENSES_KEY, 0 );
		$this->assertFieldEquals( 'UsageTerms', 'Creative Commons Attribution-Share Alike 3.0',
			$data, TemplateParser::LICENSES_KEY, 1 );
		$this->assertFieldEquals( 'UsageTerms', 'Creative Commons Attribution-Share Alike 2.5',
			$data, TemplateParser::LICENSES_KEY, 2 );

		$this->assertFieldEquals( 'LicenseUrl', 'http://www.gnu.org/copyleft/fdl.html', $data,
			TemplateParser::LICENSES_KEY, 0 );
		$this->assertFieldEquals( 'LicenseUrl', 'http://creativecommons.org/licenses/by-sa/3.0/',
			$data, TemplateParser::LICENSES_KEY, 1 );
		$this->assertFieldEquals( 'LicenseUrl', 'http://creativecommons.org/licenses/by-sa/2.5',
			$data, TemplateParser::LICENSES_KEY, 2 );
	...
	public function testLicenseTemplateInsideInformationTemplate() {
		...
		$this->assertFieldEquals( 'LicenseShortName', 'Public domain', $data,
			TemplateParser::LICENSES_KEY );
	}
	...
	public function testRestrictions() {
		...
		$this->assertFieldEquals( 'Restrictions', 'trademarked', $data,
	...
	public function testNuke() {
		...
		$this->assertFieldEquals( 'DeletionReason',
			'[[Commons:Deletion requests/File:Kerameikos October 2012 15.JPG]]: '
			. 'As far as I know tourist signs as all road signs are not in the public domain. '
			. '--[[User:G.dallorto|User:G.dallorto]] '
			. '([[User talk:G.dallorto|<span class="signature-talk">talk</span>]]) '
	 ...
	 * @param string|bool $language language code for parser's language;
	 *   false for multi-language mode
	 ...
	 */
	protected function assertFieldEquals(
		$key, $expected, $data, $type, $position = 0, $message = ''
	) {
	...
	protected function getAndAssertTemplateData( $data, $type, $position = 0 ) {
		...
		$this->assertArrayHasKey( $position, $data[$type],
			"No position $position for template type $type in data" );
	 ...
	 * @param string $message
	 */
	protected function assertFieldContainsString(
		$key, $expectedSubstring, $data, $type, $position = 0, $message = ''
	) {
		$data = $this->getAndAssertTemplateData( $data, $type, $position );
		$this->assertArrayHasKey( $key, $data, $message );
	 ...
	 * @param string $message
	 */
	protected function assertFieldNotContainsString(
		$key, $expectedSubstring, $data, $type, $position = 0, $message = ''
	) {
		$data = $this->getAndAssertTemplateData( $data, $type, $position );
		$this->assertArrayHasKey( $key, $data, $message );
	 ...
	 * @param string $message
	 */
	protected function assertFieldStartsWith(
		$key, $expectedPrefix, $data, $type, $position = 0, $message = ''
	) {
		$data = $this->getAndAssertTemplateData( $data, $type, $position );
		$this->assertArrayHasKey( $key, $data, $message );
		$actual = $data[$key];
